(() => {
  const match = location.pathname.match(/product-(els\d+)\.html$/i);
  if (!match) return;
  ...
  const colorsShown = ['C1', 'C2', 'C3', 'C4'];

  document.title = `${product.model} Acetate Sunglasses | CarpeDiem Optic`;
  document.querySelector('meta[name="description"]')?.setAttribute('content', `${product.model} acetate sunglasses by CarpeDiem Optic. Explore four photographed colourways, construction details and OEM or private label inquiry options.`);
  ...
  document.head.insertAdjacentHTML('beforeend', `<style>.main-image:before{content:'${product.model}'}.main-image:after{content:'C1 / SUN'}.main-image img{transform:none!important}</style>`);
  ...
  document.querySelector('.intro').textContent = 'An acetate sunglasses sample with a defined fashion profile and four photographed colour directions. Review the frame, lens direction and construction details as a starting point for OEM, ODM or private label development.';
  document.querySelector('.status').textContent = 'Available for project discussion';
  document.querySelector('.buyer-summary').innerHTML = '<div class="buyer-summary-item"><strong>Acetate</strong><span>Frame material</span></div><div class="buyer-summary-item"><strong>4</strong><span>Colourways shown</span></div><div class="buyer-summary-item"><strong>OEM</strong><span>Project support</span></div>';

  thumbs.innerHTML = `<button class="thumb active" type="button" data-image="${asset('front')}" data-color="Front view" data-label="Front view"><img src="${asset('front')}" alt="${product.model} acetate sunglasses front view"></button>` + colorsShown.map((code) => `<button class="thumb" type="button" data-image="${asset(code.toLowerCase())}" data-color="${code}" data-label="${code} photographed colourway"><img src="${asset(code.toLowerCase())}" alt="${product.model} ${code} acetate sunglasses"></button>`).join('') + `<button class="thumb" type="button" data-image="${asset('c1-detail')}" data-color="Construction detail" data-label="Construction detail"><img src="${asset('c1-detail')}" alt="${product.model} construction detail"></button><button class="thumb" type="button" data-image="${asset('lookbook-stack')}" data-color="Colour board" data-label="Photographed colour board"><img src="${asset('lookbook-stack')}" alt="${product.model} photographed colour board"></button>`;
  ...
  function selectImage(choice) {
    ...
    imageNote.textContent = choice.dataset.color === 'Colour board' ? 'COLOUR RANGE' : 'PRODUCT SAMPLE';
